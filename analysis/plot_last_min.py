...
def plot_psd():
    ...
    plt.clf()
    nfft=1024 #int(2**np.ceil(np.log2(sampl)));
    ...
    plt.title('Power Spectral Density (NFFT='+str(nfft)+', Fs='+str(sps)+', win=hann)')
    
    plt.savefig(os.path.join(_LOCATION_CHARTS,"last_psd.png"))
    plt.clf()
...
def plot_psd_resampled():
    ...
    plt.title('Power Spectral Density (NFFT='+str(nfft)+', Fs='+str(sps/5)+', win=hann)')
    plt.xlabel('')
