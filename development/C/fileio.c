/* compile using 
 * gcc -o test test.c -lm -lnetcdf -lrt -Wall -Wextra -Wno-missing-field-initializers -Wfatal-errors -Wunused
 */
...
int getSortedFileList(const char *datadir, const char *to_delete_file, struct timespec *fnts_p);
int copy(const char *src, const char *dest);
int notice_to_delete(char *to_delete_file, struct timespec *ts);
    ...
    char *startstr, struct timespec *fnts_p)
{
    ...
    //copy us to element of array pointer given
    fnts_p->tv_nsec = par->startEpoch.tv_usec * 1000;
    
    //purge fractional seconds    
    str[19]='\0'; 
    char *ret;
    ret=strptime(str, "%Y-%m-%d %H:%M:%S", &(par->stime));
    ...
    if (first){
        ...
        fprintf(stdout,"\tstarts at:                     %s.%06ld\n",str,par->startEpoch.tv_usec);
        printf("\tsps: %f Hz\n",par->sps);
        printf("\tchannels: %d\n",par->chnumber);
...
int getSortedFileList(const char *datadir, const char *to_delete_file, struct timespec *fnts_p)
{
    if (datadir==NULL || to_delete_file==NULL || fnts_p==NULL){
        logErrDate("%s: parameter error!\n",__func__);
        return -1;
    }
    DIR *dir;
...
/* ---------- get "to_delete.txt" file in sync --------------- */
    FILE *fil;
    //create the file, if not yet present
    if (access(to_delete_file, F_OK ) == -1) { //file does not exist
        fil=fopen(to_delete_file, "w");
        if (fil==NULL){
            logErrDate("%s: failed to open file %s to write\n",__func__,to_delete_file);
            return -1;
        }
        fclose(fil);
    ...
    char backup[255];
    sprintf(backup,"%s.bck",to_delete_file);
    copy(to_delete_file, backup);
    
    //read in
    fil=fopen(to_delete_file, "r+");
    if (fil==NULL){
        logErrDate("%s: failed to open file %s\n",__func__,to_delete_file);
        return -1;
    }
    ...
    char *content;
    content=calloc(fsize+1, sizeof(char));
    if (content==NULL){
        logErrDate("%s: could not allocate memory for reading file %s",__func__,to_delete_file);
        return -1;
    }
    fread(content,sizeof(char),fsize,fil);
    //overwrite file
    rewind(fil);
    ...
    for (i=0, j=0; i<fsize; i++){
        //get filename out of string: so for each file (not containing '\n')
        if (*(content+i)=='\n' || *(content+i)=='\0') {
            strncpy(afile, (content+j), (size_t)MIN((i-j), 255));
            afile[i-j+1]='\0';
            j=i;
            
            //if file exists, write back its name to the to_delete.txt
            if (access(to_delete_file, F_OK ) == 0) {
                fprintf(fil,"%s",afile);
                printf("%s",afile);
                k++;
            }
            else {
                printf("%s does not exists anymore.\n",afile);
            }
            
            
        }
    }
    //flush buffer here and now
    fflush(fil);
    
    fclose(fil);
    printf("\n%7ld files still exits ready for deletion.\n",k);
    free(content);
...
int copy(const char *src, const char *dest)
{
    ...
    fp1 = fopen(src, "r");
    if (fp1 == NULL) {
        logErrDate("%s: cannot open source file %s to read!\n",__func__,src);
        return -1;
    }

    fp2 = fopen(dest, "w");
    if (fp2 == NULL) {
        logErrDate("%s: cannot open destination file %s to write!\n",__func__,dest);
        fclose(fp1);
        return -1;
    }
...
{
    FILE *fid;
    fid=fopen(to_delete_file, "a+");
    if (fid==NULL){
        logErrDate("%s: failed to open file %s\n",__func__,to_delete_file);
        return -1;
    }
    if (fprintf(fid,"\n%ld.nc",ts->tv_sec)<=1){
        logErrDate("%s: could not write to file %s\n",__func__,to_delete_file);
        return -1;
    }
    return fclose(fid);      
...
int main(int argc, char * argv[])
{
    ...
    char to_delete_file[255];
    sprintf(file_in_dir, "/home/pi/data/tmp/");
    sprintf(file_out_dir,"/home/pi/data/");
    sprintf(to_delete_file,"/home/pi/data/to_delete.txt");
    ...
    i = getSortedFileList(file_in_dir, to_delete_file, &fnts[0]);
...
//printf("first file (%ld.nc) starts at:  %s\n",fnts[0].tv_sec,tmpstr); 
    ...
    if(notice_to_delete(to_delete_file, &fnts[0])){
...
// return -1;
    ...
    if (i >= 2) {
        for (j=1; j<i-1 && j<60; j++){
            if(notice_to_delete(to_delete_file, &fnts[j])){
        ...
        if (tp.tm_min != 59){
            if(notice_to_delete(to_delete_file, &fnts[i-1])){
                logErrDate("failed to delete files.\nExit\n");
                return -1;
            }
            j++; 
